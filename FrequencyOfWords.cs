...
namespace HashTable
{
    public class FrequencyOfWords<K, V>
    {
       ...
       protected LinkedList<KeyValue<K, V>> GetLinkedList(int position)
        ...
        public void GetFreq()
        {
            FrequencyOfWords<string, int> FrequencyOfWords = new FrequencyOfWords<string, int>(6);
            string[] words = { "to", "be", "or", "not", "to", "be" };
            int count = 1;
            foreach (string i in words)
            {
                count = FrequencyOfWords.CheckHash(i);
                if (count > 1)
                {
                    FrequencyOfWords.Add(i, count);
                }
                else
                {
