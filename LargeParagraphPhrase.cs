...
namespace HashTable
{
    public class LargeParagraphPhrase<K, V>
    {
        private readonly int size;
        private readonly LinkedList<KeyValue<K, V>>[] items;
        public LargeParagraphPhrase(int size)
        {
            this.size = size;
        ...
        public void GetFreq()
        {
            LargeParagraphPhrase<string, int> LargeParagraphPhrase = new LargeParagraphPhrase<string, int>(6);
            string[] words = { "to", "be", "or", "not", "to", "be" };
            int count = 1;
            foreach (string i in words)
            {
                count = LargeParagraphPhrase.CheckHash(i);
                if (count > 1)
                {
                    LargeParagraphPhrase.Add(i, count);
                }
                else
                {
                    LargeParagraphPhrase.Add(i, 1);
                }
            }
            ...
            {
                LargeParagraphPhrase.Display(i);
            }
        }
